...
export function VolunteerOverview({
  ...
  const eventsWithRecentPosts = approvedEvents
    .map((event) => {
      ...
      const latestPost = eventPosts.sort(
        (a, b) =>
  ...
  const trendingEvents = approvedEvents
    .map((event) => {
      ...
      const engagementScore =
        totalLikes * 2 + totalComments * 3 + recentRegistrations * 10;
      return {
        event,
        eventPosts,
        totalLikes,
  ...
  const upcomingSchedule = myRegistrations
    .filter(
      (e) =>
        ...
        e.registrations.some(
          (r) => r.user.id === currentUser.id && r.status === "APPROVED"
        )
    )
    .sort(
      (a, b) =>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Here's what's happening in your volunteer journey
        </p>
      </div>
                          ...
                          {totalComments > 0 && (
                            <div className="flex items-center gap-2 text-sm">
                              ...
                              <span className="text-blue-600 font-medium">
                                {totalComments} active discussions
                              </span>
                            </div>
                          )}
                        </div>
